...
Please note that for the GTM body snippet to fire correctly, your theme must call `<?php wp_body_open(); ?>` just after the opening `<body>` tag. Most modern themes (WordPress 5.2+) already support this. If yours does not, contact your theme developer. Here is some code to <a href="https://gist.github.com/ronalfy/6b7f43e70e644320aba67d1876a0aeff">show the theme developer to make it compatible with Metronet Tag Manager</a>.
...
https://wpmetronet.com/how-to-track-pdf-downloads-with-tag-manager-on-wordpress/

The Benefits of Using a Tag Manager Plugin on WordPress
https://wpmetronet.com/the-benefits-of-using-a-tag-manager-plugin-on-wordpress/

Metronet Tag Manager is maintained by Ruskin Consulting (https://ruskinconsulting.com). If you need help with Google Tag Manager, Google Analytics (GA4), or analytics strategy for your WordPress site, reach out to us — we would love to help.
...
> <strong>Note</strong>: if your theme does not support wp_body_open, contact your theme developer and point them to this <a href="https://gist.github.com/ronalfy/6b7f43e70e644320aba67d1876a0aeff">compatibility snippet</a>.
...
Yes. Use %placeholder% syntax in variable values. Many placeholders are built in. For custom dynamic values, see the Filtering Variables section.
...
* %wordcount% — Word count of the post content
* %logged_in% — true or false depending on login state
* %page_id% — The WordPress queried object ID
* %post_id% — The numeric post ID
* %post_date% — The post publication date
* %post_type% — The post type slug (e.g. post, page)
* %category% — The primary category slug
* %tags% — Comma-separated tag slugs
* %permalink% — The canonical URL of the post/page
* %language% — The site locale string (e.g. en_US)

= What is Google Consent Mode v2? =

Google Consent Mode v2 is required for EU sites using GTM with Google Ads or Analytics. It lets you set default consent states before GTM fires, ensuring compliant data collection even before a user interacts with your consent banner. Enable it under Settings > Metronet Tag Manager > Consent Mode.

= How do I exclude admins from GTM tracking? =

Under Other Options, check "Exclude logged-in users by role" and select which roles to exclude. GTM snippets will not be output for those users.

= How do I migrate settings between sites? =

Use the Export Settings button to download a JSON file of your current configuration. On the target site, use Import Settings to upload and apply it.
...
* New feature: Gutenberg dataLayer Push block (metronettagmanager/datalayer-push) — add button-triggered dataLayer events in the block editor.
* Maintenance: Updated all JS/CSS dependencies flagged by Dependabot (axios, node-sass, jquery, loader-utils, qs, and others).
* Maintenance: WPCS 3.0 compliance — renamed getMessage() to get_message(), fixed remaining coding standards violations.
* Tested with WordPress 6.9.

= 1.5.5 =
* Released 2023-06-24
* Adding ability to update header message remotely.
...
= 1.0.4 =
* Released 2014-08-27
* Fixed meta problem with data variables showing up in areas where there aren't posts or pages.
...
The filter name is `gtm_` with the suffix of `replace_test`, since that content is between the percentage signs. Each filter callback receives three arguments: the full match string, the match name, and the current post ID.
...
Maintained by Ruskin Consulting (https://ruskinconsulting.com). Need help with Google Tag Manager or analytics? Get in touch at https://ruskinconsulting.com.
