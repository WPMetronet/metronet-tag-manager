...
class Metronet_Tag_Manager {
	...
	/**
	 * Returns a boolean if the site is multisite or not
	 *
	 * @return bool True if plugin is network activated, false if not
	 */
	...
	public function init() {
		...
		// Load GTM in the Footer (or header if they have the do_action( 'body_open' )
		// or use `wp_body_open` if user is using 5.2 and up functionality
	...
	public function meta_box_save( $post_id ) {
		...
		// Verify da nonce
		if ( ! isset( $_POST['mtm_save_post'] ) ) {
			return;
		}
		if ( ! wp_verify_nonce( $_POST['mtm_save_post'], plugin_basename( __FILE__ ) ) ) {
			return;
		}
	...
	public function output_tag_manager_head() {
		...
		foreach ( $data_layer_array as &$value ) {
			if ( preg_match( '/^%([^%]*)%/', $value, $matches ) ) {
				 ...
				 * @since 1.0.0
				 *
				 * @param string  $matches0 %variable_name%
				 * @param string  $matches1 variable_name
	...
	public function output_tag_manager_body() {
		if ( did_action( 'fl_body_open' ) === 1 && did_action( 'wp_footer' ) === 1 ) {
			return;
		}
		if ( did_action( 'wp_body_open' ) === 1 && did_action( 'wp_footer' ) === 1 ) {
			return;
		}
		if ( did_action( 'body_open' ) === 1 && did_action( 'wp_footer' ) === 1 ) {
			return;
		}
	 ...
	 * @return array Appended array of CSS attributes
	 */
	 ...
	 * @return string A formatted variable
	 */
